...
router.get('/edit/:id', isAuth, (req, res) => {
    ...
    Habit.find({userId: req.user._id}, (err, habit) => {
...
//             });
//     newHabit.save()
//         .then(res.redirect('/habit'))
//         .catch (err => console.log(err))
// })

router.post('/edit/:id', (req, res) => {
    const id = req.params.id
    Habit.findByIdAndUpdate(id, { content: req.body.content }, err => {
        if (err) return res.send(500, err)
        res.redirect("/habit")
    })
})

//Updates Status of Habit Completion
